...
def delete_file_api(file_id: int, user_id: int, role: str, username: str):
    try:
        ...
        cur.execute("""
            SELECT f.file_name, f.file_path, u.username
            FROM files f
            JOIN users u ON f.uploaded_by = u.id
            WHERE f.id = %s
        """, (file_id,))
        ...
        file_name, storage_path, uploaded_by = file_data
        ...
        # 1. Delete the real file from Supabase Storage
        supabase.storage.from_(SUPABASE_BUCKET).remove([storage_path])

        # 2. Delete the file record from PostgreSQL
        ...
        return {
            "success": True,
            "message": "File deleted from database and storage successfully",
            "file_name": file_name
        }
