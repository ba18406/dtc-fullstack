...
def create_page_header(parent, title, subtitle, current_user=None):
    header = tk.Frame(parent, bg=PRIMARY_COLOR, height=100)
    header.pack(fill="x")
    header.pack_propagate(False)

    left = tk.Frame(header, bg=PRIMARY_COLOR)
    ...
    center = tk.Frame(header, bg=PRIMARY_COLOR)
    center.pack(side="left", padx=50, pady=12)
    ...
    if current_user:
        right = tk.Frame(header, bg=PRIMARY_COLOR)
...
def open_dashboard(current_user):
    ...
    if current_user["role"] == "admin":
        sidebar_button(
            "📊  Activity Logs",
            lambda: show_logs_page(main_content, current_user)
        )
...
def show_logs_page(main_content, current_user):
    clear_main_content(main_content)

    create_page_header(
        main_content,
        "Activity Logs",
        "Complete audit trail and system monitoring",
        current_user
    )
...
form = tk.Frame(content, bg="#f1f5f9")
form.pack()
