...
def get_visible_users(current_user):
    try:
        response = requests.get(
            f"http://127.0.0.1:8000/users/visible/{current_user['id']}/{current_user['role']}"
        )
...
def get_supervisors_for_dropdown():
    try:
        response = requests.get("http://127.0.0.1:8000/supervisors")
        result = response.json()
...
def delete_user_from_db(user_id):
    response = requests.delete(
        f"http://127.0.0.1:8000/users/delete/{user_id}"
    )
...
def get_dashboard_statistics(current_user):
    response = requests.get(
        f"http://127.0.0.1:8000/dashboard/stats/{current_user['id']}"
    )
...
def get_recent_uploads(limit=5):
    response = requests.get(
        f"http://127.0.0.1:8000/uploads/recent?limit={limit}"
    )
...
def get_visible_files(current_user):
    try:
        response = requests.get(
            f"http://127.0.0.1:8000/files/visible/{current_user['id']}/{current_user['role']}/{current_user['username']}"
        )
...
def show_files_dashboard(current_user):
    try:
        ...
        def delete_file():
            ...
            try:
                response = requests.delete(
                    f"http://127.0.0.1:8000/files/delete/{file_id}/{current_user['id']}/{current_user['role']}/{current_user['username']}"
                )
...
def show_files_page(main_content, current_user):
    ...
    def open_selected_file(event=None):
        ...
        try:
            preview_file_inside_tkinter(file_path)
