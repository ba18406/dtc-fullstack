...
SUPABASE_URL = "https://dlqfrjjnkdtphjgdynpt.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRscWZyampua2R0cGhqZ2R5bnB0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzgxMzc1MTUsImV4cCI6MjA5MzcxMzUxNX0.44m6Njt4hm7QPAMDrlpRWKlxtvTwjCjPzQn2Ktt6JX8"
...
def open_dashboard(current_user):
    ...
    recent_tree.heading("uploaded_at", text="Upload Date")

    recent_tree.column("file_name", width=250)
    recent_tree.column("username", width=150, anchor="center")
    recent_tree.column("uploaded_at", width=180, anchor="center")
...
def upload_file(current_user):
    ...
    cur.execute("""
        ...
        VALUES (%s, %s, %s, %s)
    """, (file_name, file_type, file_url, current_user["id"]))
...
def upload_file_to_supabase(file_path, file_name):
    ...
    safe_file_name = f"{int(time.time())}{ext}"

    with open(file_path, "rb") as f:
        response = supabase.storage.from_("dtc-files").upload(
            path=safe_file_name,
            file=f,
            file_options={"content-type": "application/octet-stream"}
        )

    file_url = supabase.storage.from_("dtc-files").get_public_url(safe_file_name)
